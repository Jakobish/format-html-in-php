...
} } function AddNewLinkR() { if (document.people.TextReplace.value=='' ) { alert('נא מלא את המילה להחלפה'); } else {
if (document.people.linkReplace.value=='' ) { alert('נא מלא את הקישור להחלפה'); } else {
...
ךרע יפל רושיק</a><br>
<br>
<a href="javascript:openWindow('AddFeedback.asp?docId=<%=request("docId")%>&subjectId=<%=request("SubjectId")%>','rates')">
בושמ לוהינ</a><br>
<!--br>
...
sSql = "SELECT * FROM T_links WHERE" & _ _
" docId=" & iDocId & " AND " & _ _
...
<td dir="RTL">
<input type="text" name="decease_date" value="<%=idecease_date%>" tabindex="24" size="20">
</td>
<td>
...
<%else%>
<td dir="RTL">
<input type="text" name="decease_date" value="<%=request("decease_date")%>" tabindex="24" size="20">
</td>
<td>
<font size="2">:<span lang="he">יתחפשמ בצמ</span></font>
</td>
...
<%
set rsChiled=Hiconn.execute("select* _
from T_AreaCategories where parentID="&rsBig("catID")&" order by catname")
...
librs.open libsql,Hiconn
%>
<select size="3" name="Skill_Parent_ID" dir="rtl" style="width: 120" onchange="manuselected1(this);">
<%
do until librs.eof
%>
...
<%else%>
<td dir="LTR" align="right">
<input type="text" name="site_url" value="<%=site_url%>" size="20" tabindex="32">
</td>
<%end if
end if%>
<td nowrap>
...
<td dir="LTR" valign="top">
<input type="text" name="person_salary" value="<%=sSalary%>" size="20" tabindex="36">
</td>
<td valign="top" nowrap>
