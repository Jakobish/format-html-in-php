...
export interface VbscriptOptions {
  indentSize: number;
  indentChar: string;
...
interface ParseContext {
  indentLevel: number;
  inMultiLineStatement: boolean;
  inSelectCase: boolean;
  inWithBlock: boolean;
  pendingIndentIncrease: boolean;
  currentBlockType: string;
}

export default function beautifyVbscript(code: string, options: VbscriptOptions): string {
  ...
  // ASP-specific keywords and constructs
...
function formatArraysAndObjects(line: string, indent: string, indentLevel: number): string {
...
function formatAspConstructs(line: string, indent: string, indentLevel: number): string {
  ...
  line = line.replace(/Response\.Write\s*\(\s*/gi, 'Response.Write(');

  // Format Request object access
  line = line.replace(/Request\s*\.\s*/gi, 'Request.');

  // Format Session and Application objects
  line = line.replace(/(Session|Application)\s*\.\s*/gi, '$1.');

  // Format SQL queries with proper spacing
  line = line.replace(/(\w+)\s*=\s*"(.*SELECT.*FROM.*)"/gi, (match, varName, sql) => {
    return `${varName} = "${sql.trim()}"`;
  });

  // Format recordset operations
  line = line.replace(/(\w+)\s*\.\s*(EOF|BOF|MoveNext|MoveFirst|MoveLast)/gi, '$1.$2');

  // Format error handling
  line = line.replace(/On\s+Error\s+/gi, 'On Error ');

  // Format ASP directives and includes
...
function breakLongLine(line: string, maxLength: number, indent: string, indentLevel: number): string {
...
function applyAssignmentAlignment(lines: string[], assignments: Array<{ lineIndex: number; assignmentPos: number; line: string }>, indentSize: number): string {
